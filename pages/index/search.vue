<template>
	<view class="search_list">
		<u-sticky>
			<view>
				<view class="flex-left-right white-back c-search">
					<view class="search-text ">
						...
						<view class="flex-center" style="margin-left: 54rpx;">
							<text class="iconfont icon-sousuo"></text>
							<input v-model="keyword" placeholder="搜索你想知道的" placeholder-class="fs-12" class="input" />
						</view>
					</view>
					<view>
						<text @click="submit">搜索</text>
			...
			<view>
				<view class="input_key" v-if="condition=='商品'">

					<view class="flex-left-right fs-15 sort" v-show="!showSearch&&list.length">
		...
		<view class="search_result" v-else>
			<block v-if="condition=='商品'">
				<view :class="open?'recomend-one':'recomend'">
					<view v-for="(item,index) in list" :key="index" :style="{'width':open?'100%':'330rpx'}">
						<my-recommend showCar="true" :open="open"></my-recommend>
					</view>
				</view>
			...
			<block v-else>
				<view style="margin-top:-30rpx;">
					<view class="browse-article padding-15 white-back" v-for="(item,index) in 12" :key="index">
