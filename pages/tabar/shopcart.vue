<template>
	<view>
		...
		<u-navbar back-text="删除" back-icon-size="0" :back-text-style="color" @custom-back="delterGood" title="医来顺"
			:background="background" title-color="white"></u-navbar>
		<view class="shopcart" v-if="itemList.length!=0">
			<view class="good-num white-back">
				<view class="padding-15">
					<u-checkbox-group @change="checkboxGroupChange">
						<u-checkbox v-model="item.checked" v-for="(item, index) in itemList" :key="index"
							:name="item.id" shape="circle" class="border">
							<view class="my_profile flex u-flex-1">
									...
									<view class="flex-left-right">
										<view style="width:82%;" @click="$doubleClick(detail)">
											<view class="hidden">
												...
												<text class="gray-font">/{{item.sku.unit}}</text>
											</view>
										</view>
										<u-number-box v-model="item.buy_num" @change="valChange"></u-number-box>
				...
				<!-- <view class="flex-left-right go-bill fs-11" v-if="feeInfo.over>totalPrice">
					...
					<navigator open-type="switchTab" url="/pages/tabar/classify">
						<view class="red-font flex-center">
							<text>去凑单</text>
			...
			<view class="new-order">
				<view class="flex-left-right submit white-back">
					<view class="flex-center">
						<u-checkbox @change="allCheckGood" v-model="allCheck" shape="circle">全选</u-checkbox>
					</view>
					<view class="flex-center">
						<text class="fs-13 gray-font">不含运费 合计：</text>
						<text class="fs-18 red-font">￥{{$fixed(totalPrice)}}</text>
						<text class="fs-15 gray-font pay-button" :class="settlement?'go-settle':'in-go-settle'"
		...
		<view v-else class="null_cart">
			...
			<view class="hot-sale">
				<view class="name flex">
					...
					<view class="fs-17 bold" style="margin:0 8rpx;">浏览记录</view>
					<view class="flex-center">
						<text></text>
						<text class="orange"></text>
					</view>
